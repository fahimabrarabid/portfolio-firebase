...
import { addData, fetchData } from '@/configs'
  ...
  const handleRemoveUserAdmin = async (uid) => {
    ...
    try {
      ...
      setAdminList((prevAdminList) =>
        prevAdminList.filter((admin) => admin.uid !== uid)
      )
    } catch (error) {
  ...
  return (
    <div className="user-list w-3/4 m-auto mt-10">
      ...
      {googleUsers.map((user) => (
        <div
          className="user-card bg-white rounded-md shadow-md p-4 mb-4"
          key={user.uid}
        >
          ...
          {currentUser.uid !== user.uid && (
            <div className="button-group mt-4">
              {isAdmin(user.uid) ? (
                <button
                  className="admin-button bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded"
                  onClick={() => handleRemoveUserAdmin(user.uid)}
                >
                  Remove Admin
                </button>
              ) : (
                <button
                  className="admin-button bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-2"
